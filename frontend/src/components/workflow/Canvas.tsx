...
export function Canvas({
  ...
  return (
    <div
      ref={wrapperRef}
      className="relative h-full overflow-hidden rounded-md border border-border bg-slate-950/60"
    ...
    >
      ...
      <div className="absolute right-3 top-3 rounded border border-border bg-background/80 px-2 py-1 text-xs text-muted-foreground">
        缩放 {Math.round(viewport.zoom * 100)}% · Shift + 拖拽框选 · 中键平移
      </div>
    </div>
  )
}
