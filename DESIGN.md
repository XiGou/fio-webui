...
- **Execution node:** Fixed-width white module on the signal path with operation number, name, concurrent Job count, visible jobs, and local add/move/delete tools. Selected state uses the primary border and halo, and exposes an insertion control on its outgoing connector; the module palette inserts after the same selected node while the terminal plus always appends. One node compiles to one fio task; nodes execute in canvas order. Dragging any blank ruled-canvas area pans the viewport without competing with node controls.
...
- **Inspector:** The right workbench edits exactly one scope at a time. Selecting a node exposes shared values; selecting a Job exposes only that Job's inherited values and explicit overrides. Its equipment header, dividers, icon well, field density, and state vocabulary match the module palette.
...
- **Metric evidence:** IOPS, bandwidth, and latency always show units and useful context. Active metrics name `fio stdout/status`, open in a one-minute sliding window with now pinned to the right edge, and expose explicit `30s / 1m / 5m / 15m / all` observation ranges with a follow-latest state. WebSocket samples are backed by periodic persisted-stat refresh when the stream is stale. Every sample retains stage and Job identity; Job names are unique within a node. New runs persist per-Job `taskN_*.<job-number>.log` files, while aggregate log names remain a compatibility input. Realtime and report charts share Job, read/write direction, and latency mean/p99/max multi-select controls. Reports show per-Job summaries, every execution node boundary, and re-range the y axis when a series is hidden. `stats.jsonl` is labeled only as a compatibility fallback.
