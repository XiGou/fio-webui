...
- **Metric evidence:** IOPS, bandwidth, and latency always show units and useful context. Active metrics name `fio stdout/status`, open in a one-minute sliding window with now pinned to the right edge, and expose explicit `30s / 1m / 5m / 15m / all` observation ranges with a follow-latest state. WebSocket samples are backed by periodic persisted-stat refresh when the stream is stale. Reports name the persisted `taskN_iops.log`, `taskN_bw.log`, `taskN_lat.log`, and `taskN_clat_hist.log` sources. Report charts use uPlot, show every execution node boundary, expose IOPS/BW mean and max plus completion-latency mean, p99, and max, and re-range the y axis when a series is hidden. `stats.jsonl` is labeled only as a compatibility fallback.
