...
- **Execution node:** Fixed-width white module on the signal path with operation number, name, concurrent Job count, visible jobs, and local add/move/delete tools. Selected state uses the primary border and halo. One node compiles to one fio task; nodes execute in canvas order. Dragging any blank ruled-canvas area pans the viewport without competing with node controls.
